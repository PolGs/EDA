...
vector<vector<double>> updatewmatrixgas(Pos carpos, int cid){
    ...
    for(int i=0; i<n; ++i){
        for(int j=0; j<m; ++j){
             Pos p(i,j);;
            if(cell(p).type == WATER_BONUS or cell(p).type == GAS_BONUS){
                a.pos = p;
                a.val = 5;
		if(cell(p).type == WATER_BONUS) a.val = 0;
		if(cell(p).type == GAS_BONUS and car(cid).gas < 80)) a.val = 0;
